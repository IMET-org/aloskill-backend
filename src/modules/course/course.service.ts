  ...
  const formatCourseData = (course: typeof getCourseDetails) => {
    ...
    course.modules.forEach(module => {
      module.lessons.forEach(lesson => {
        if (lesson.type === 'VIDEO') {totalVideoCount++;}
    ...
    return {
      ...
      courseInstructors: course.courseInstructors.map(i => ({role: i.role, displayName: i.instructor.displayName, avatarUrl: i.instructor.user.avatarUrl})),
