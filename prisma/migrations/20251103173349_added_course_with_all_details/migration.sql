/*
  ...
  - You are about to alter the column `country` on the `UserSession` table. The data in that column could be lost. The data in that column will be cast from `Text` to `Char(2)`.
...
-- CreateEnum
CREATE TYPE "TeachingApproach" AS ENUM ('INTERACTIVE', 'VIDEO', 'LIVE', 'ASSIGNMENT');
...
-- DropIndex
DROP INDEX "public"."UserSession_lastActivity_idx";

-- DropIndex
DROP INDEX "public"."UserSession_userId_idx";

-- AlterTable
ALTER TABLE "UserSession" ADD COLUMN     "geoLocation" JSONB,
ADD COLUMN     "suspicious" BOOLEAN NOT NULL DEFAULT false,
ADD COLUMN     "suspiciousSince" TIMESTAMP(3),
...
CREATE TABLE "Quiz" (
    ...
    "description" TEXT,
    "duration" INTEGER,
    "passingScore" DECIMAL(3,2) NOT NULL,
...
-- CreateIndex
CREATE INDEX "InstructorProfile_isVerified_idx" ON "InstructorProfile"("isVerified");
